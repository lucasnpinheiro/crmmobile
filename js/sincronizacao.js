...
_sincronicacao = {produtos: {qtdPaginacao: 25, qtdAtual: 0, qtdMax: 0, sequencia: 0, insert: function(a) {
            var b = 0;
            var c = a.length;
            $.each(a, function(e, d) {
                var f = 'INSERT OR REPLACE INTO produtos (id_produtos, cod_produto, dsc_produto, desconto_maximo, data_hora_atualizacao, estoque, valor, id_empresas) VALUES ("' + d.id_produtos + '","' + d.cod_produto + '","' + d.dsc_produto + '","' + d.desconto_maximo + '","' + date("Y-m-d H:i:s") + '","' + d.estoque + '",' + d.valor + ',"' + d.id_empresas + '");';
                debug("QUERY", f);
            ...
            $.ajax({url: _situacoes.urls.produtos_total, dataType: "html", type: "GET", beforeSend: function() {
                    tcount.start();
                    block(false);
            ...
            $.ajax({url: _situacoes.urls.produtos_lista, dataType: "json", type: "GET", data: {inicio: _sincronicacao.produtos.sequencia + 1, qtde: _sincronicacao.produtos.qtdPaginacao}, beforeSend: function() {
